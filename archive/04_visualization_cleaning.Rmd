# Visualization, Data Cleaning, and Joining

## Learning objectives

By the end of this chapter, you should be able to reshape population data, create basic visualizations, identify join keys, clean mortality records, work with missing values, create date variables, categorize records, create exposure variables, and write cleaned outputs.

## Chapter overview

This chapter combines visualization and data-cleaning practice. It keeps the original workflow but organizes the material into a clearer sequence from review, to cleaning, to categorization, to joins and activities.

# Load Libraries
```{r ch04-Read-Libraries}
library(tidyverse)
library(ggplot2)
library(readxl)
library(lubridate)
...
```{r ch04-pop_wide, include=FALSE, eval=FALSE}
...
```{r ch04-write-exposure}
write_csv(env, "./Outputs/exposures.csv")
```
