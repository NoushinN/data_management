# Strings, Regular Expressions, and Reshaping

## Learning objectives

By the end of this chapter, you should be able to clean character variables, split strings into useful columns, extract patterns with regular expressions, create a simple helper function, and reshape the WHO dataset into tidy format.

## Chapter overview

This chapter uses the Titanic dataset and the built-in `who` dataset to practise `tidyr` and `stringr`. The original practice content is preserved, with minor corrections to make the workflow run smoothly in bookdown.

# Load Libraries

```{r ch03-load_libraries}
library(tidyverse)
library(stringr)
